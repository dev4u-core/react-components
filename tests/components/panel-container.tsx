...
describe('<PanelContainer />', () => {
    ...
    describe('behaviour', () => {
        it('panel should be closed (dynamic with 3 panel)', () => {
            ...
            let panelContainer = Enzyme.mount(<PanelContainer dynamicPanels={dynamicPanels} />);
        ...
        it('panel should not be closed (dynamic with 3 panel)', () => {
            let dynamicPanels = [{ title: 'panel0' }, { title: 'panel1' }, { title: 'panel2' }];
            let panelContainer = Enzyme.mount(<PanelContainer dynamicPanels={dynamicPanels} onPanelClosing={() => false} />);
        ...
        it('panel should be closed (static with 1 panel)', () => {
            let panelContainer = Enzyme.mount(<PanelContainer onPanelClosing={() => true}><Panel /></PanelContainer>);
            panelContainer.find('button.close').last().simulate('click');

            expect(panelContainer.state().staticPanels.length).to.equal(0);
        });
        it('panel should be closed (static with 2 panels)', () => {
            let panelContainer = Enzyme.mount(<PanelContainer onPanelClosing={() => true}><Panel /><Panel /></PanelContainer>);
            panelContainer.find('button.close').last().simulate('click');

            expect(panelContainer.state().staticPanels.length).to.equal(1);
        });
        it('panel should not be closed (static with 1 panel)', () => {
            let panelContainer = Enzyme.mount(<PanelContainer onPanelClosing={() => false}><Panel /></PanelContainer>);
            panelContainer.find('button.close').last().simulate('click');

            expect(panelContainer.state().staticPanels.length).to.equal(1);
        });
        it('panel should not be closed (static with 2 panels)', () => {
            let panelContainer = Enzyme.mount(<PanelContainer onPanelClosing={() => false}><Panel /><Panel /></PanelContainer>);
            panelContainer.find('button.close').last().simulate('click');
